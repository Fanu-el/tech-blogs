              ...
              {post.content
                ...
                .map((block, index) => {
                    ...
                    return (
                      <ul key={index} className="list-disc list-inside space-y-1">
                        {items.map((item, itemIndex) => (
                          <li key={itemIndex}>{parseInlineMarkdown(item.replace("- ", ""))}</li>
                        ))}
                      </ul>
                    )
                  }
                  ...
                  if (block.startsWith("```")) {
                    ...
                    return (
                      <pre key={index} className="bg-slate-900 text-slate-100 p-4 rounded-lg overflow-x-auto">
                        <code>{code}</code>
                      </pre>
                    )
                  }
                  ...
                  // Regular paragraphs
                  return (
                    <p key={index}>{parseInlineMarkdown(block)}</p>
                  )
                })}
            </section>
